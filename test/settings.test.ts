...
// test exercises that exact merge shape to confirm stale keys survive as
// harmless extra properties rather than causing a load error or clobbering
// unrelated settings.
// ---------------------------------------------------------------------------

describe('legacy settings key tolerance', () => {
	/** A `data.json` payload written by a pre-#106 plugin version. */
	const legacyRaw = {
		authType: 'apiKey',
	...
	it('keeps other DEFAULT_SETTINGS fields intact when raw omits them', () => {
		...
		expect(merged.synapseFolder).toBe(DEFAULT_SETTINGS.synapseFolder);
	});
