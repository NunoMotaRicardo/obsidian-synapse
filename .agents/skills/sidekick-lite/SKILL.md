...
A lighter-weight version of `/sidekick-build` for small, low-risk changes. Skips the GitHub
issue, the sidekick-technical-planner, and the sidekick-reviewer — but **not** verification.
See `wiki/decisions/2026-06-14-github-issue-workflow.md`.

Only run this cycle when the user explicitly asks for "lite" — default to `/sidekick-build`.

## Steps

1. Spawn `sidekick-coder` (foreground, lite mode) with the plain-text description. It:
   - creates/checks out `claude/<slug>`
   - implements in a single pass
   - builds, lints, and deploy-tests (`.claude/skills/deploy-test/`) — required even in lite
     mode, only the issue/reviewer ceremony is skipped
...
- If the change turns out to be bigger than expected mid-implementation, stop and tell the user
  it probably needs `/sidekick-build` instead.
