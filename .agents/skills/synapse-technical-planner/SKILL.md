...
description: Translate functional intent (.docs/decisions/) into authoritative technical specs in specs/ and GitHub issues on the repo origin points to. Use to audit a request (an existing issue or a feature description), decide whether it fits one coding cycle or must be split, and own specs/<module>.md updates. Requires gh CLI auth.
---
...
`specs/` and the repository's **GitHub issues** on the repo `origin` points to — there is
...
`specs/`/`src/` files, spawn a generic `Explore` agent for the read-only sweep, then plan
...
`specs/<module>.md` as part of its diff — run the **deploy-test** skill after merge to
