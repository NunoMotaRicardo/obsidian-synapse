...
export class UserInputModal extends Modal {
	...
	onOpen(): void {
		...
		contentEl.createEl('h3', {cls: 'synapse-modal-title', text: 'Synapse needs your input'});
		...
		if (this.request.choices && this.request.choices.length > 0) {
			const choicesContainer = contentEl.createDiv({cls: 'synapse-userinput-choices'});
			for (const choice of this.request.choices) {
				const btn = choicesContainer.createEl('button', {cls: 'synapse-userinput-choice', text: choice});
		...
		// Freeform text input
		if (allowFreeform) {
			const inputContainer = contentEl.createDiv({cls: 'synapse-userinput-freeform'});
			const input = inputContainer.createEl('textarea', {
				cls: 'synapse-userinput-textarea',
				attr: {placeholder: 'Type your answer…', rows: '3'},
			});

			const btnRow = inputContainer.createDiv({cls: 'synapse-userinput-buttons'});
