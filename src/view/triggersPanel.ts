...
export function installTriggersPanel(ViewClass: {prototype: unknown}): void {
	...
	proto.buildTriggersPanel = function (parent: HTMLElement): void {
		// ── History section (top) ─────────────────────────────
		const historySection = parent.createDiv({cls: 'synapse-triggers-section synapse-triggers-history-section'});
		const historyHeader = historySection.createDiv({cls: 'synapse-triggers-header'});
		historyHeader.createDiv({cls: 'synapse-triggers-title', text: 'History'});

		const historyControls = historyHeader.createDiv({cls: 'synapse-triggers-controls'});

		// Filter by name
		const historySearchEl = historyControls.createEl('input', {
			type: 'text',
			placeholder: 'Filter…',
			cls: 'synapse-triggers-search',
		});
		...
		const agentFilterBtn = historyControls.createEl('button', {
			cls: 'clickable-icon synapse-triggers-ctrl-btn',
		...
		this.triggerHistoryListEl = historySection.createDiv({cls: 'synapse-triggers-list'});

		// ── Configured triggers section (bottom) ──────────────
		const configSection = parent.createDiv({cls: 'synapse-triggers-section synapse-triggers-config-section'});
		const configHeader = configSection.createDiv({cls: 'synapse-triggers-header'});
		configHeader.createDiv({cls: 'synapse-triggers-title', text: 'Configured triggers'});

		const configControls = configHeader.createDiv({cls: 'synapse-triggers-controls'});
		...
		this.triggerConfigListEl = configSection.createDiv({cls: 'synapse-triggers-list'});
	};
	...
	proto.renderTriggerHistory = function (): void {
		...
		if (items.length === 0) {
			this.triggerHistoryListEl.createDiv({cls: 'synapse-triggers-empty', text: 'No trigger history yet.'});
			return;
		}
	...
	proto.renderTriggerConfigList = function (): void {
		...
		if (items.length === 0) {
			this.triggerConfigListEl.createDiv({cls: 'synapse-triggers-empty', text: 'No triggers configured.'});
			return;
		}

		for (const trigger of items) {
			const item = this.triggerConfigListEl.createDiv({cls: 'synapse-session-item synapse-triggers-config-item'});
			...
			setIcon(iconEl, 'zap');
			if (trigger.enabled) {
				iconEl.createSpan({cls: 'synapse-triggers-enabled-dot'});
			}
			...
			details.createDiv({cls: 'synapse-session-name', text: displayName});
			...
			details.createDiv({cls: 'synapse-session-time', text: scheduleText});
	...
	proto.initTriggerScheduler = function (): void {
		...
		const scheduleFileChangeCheck = (filePath: string) => {
			if (filePath.startsWith(synapseFolder + '/') || filePath.startsWith('.synapse-attachments/')) {
				debugTrace(`Synapse: ignoring change in excluded folder: ${filePath}`);
				return;
			}
			...
			fileChangeTimer = setTimeout(() => {
				...
				for (const p of paths) {
					debugTrace(`Synapse: vault file-change event (debounced): ${p}`);
	...
	proto.fireTriggerInBackground = async function (trigger: TriggerConfig, context?: TriggerFireContext): Promise<void> {
		if (!this.plugin.copilot) {
			console.warn('Synapse: trigger skipped — no copilot client available');
			return;
		}
		...
		try {
			...
			console.error('Synapse: trigger failed', trigger.name, e);
