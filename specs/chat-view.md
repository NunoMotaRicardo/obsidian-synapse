...
- **Top state line & split toolbar layout:** The top state line (`.synapse-state-line`) houses context-and-scope controls positioned directly before the active note: the working directory button (`.synapse-cwd-btn`), scope icon button (`.synapse-f-btn-scope`, folder icon), and attach icon button (`.synapse-f-btn-attach`, paperclip icon), followed by the active note name (`.synapse-state-note`). Scope and Attach buttons are icon-only without text labels. The bottom toolbar is split in two rows: the upper config toolbar (`.synapse-config-toolbar`) houses session configuration controls (Agent, Model, Reasoning Effort, Tools Approval) on the left and the compact 24px Send button on the right; the lower toolbar (`.synapse-config-toolbar-lower`) houses only the context-window gauge on the left and Debug toggle on the right, eliminating horizontal crowding in narrow panels while maintaining a clean editorial layout.
