...
- **Capabilities** — Claude Brain base folder path and **Initialize** button (creates default subfolders, sample skill, MCP configuration, sample prompt, daily planner trigger, and default methodology agents `general.agent.md`, `vision.agent.md`, `zettelkasten.agent.md`, `para.agent.md`, `lyt.agent.md`). Also includes editor integration toggles (auto-update working directory, auto-include note images, and max note images).
