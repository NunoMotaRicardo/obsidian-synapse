...
export default tseslint.config(
	...
	{
		files: ['src/agentService.ts', 'src/sdkShims.ts'],
		rules: {
			...
			// rationale does not apply to a deliberate global patch. (The shim lives in
			// `sdkShims.ts` since #236 — same rule, same rationale.)
			'obsidianmd/no-global-this': 'off',
		},
	},
	globalIgnores([
		...
		"versions.json",
		"main.js",
		// Generated by `npm run test:coverage` (#146). Gitignored, but eslint does not
		// read .gitignore, so without this a local coverage run makes the very next
